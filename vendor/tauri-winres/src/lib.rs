...
impl WindowsResource {
    ...
    pub fn compile(&self) -> io::Result<()> {
        // ANDRII build patch: skip native Windows resource compilation.
        //
        // Upstream shells out to `windres` (MinGW binutils) on the *-windows-gnu
        // toolchain to embed an icon/version resource into the .exe. That is the
        // ONLY external tool the otherwise self-contained rustup gnu toolchain
        // needs, and it forces a MinGW/MSYS2 install. Skipping it lets the project
        // build with rustup alone — no MinGW, MSVC, windres, MSYS2 or Windows SDK.
        //
        // Nothing functional is lost: the application window icon is provided at
        // runtime from tauri.conf.json, and the installer icon comes from the
        // bundled .ico. Only the icon/version metadata embedded directly in the
        // .exe file is omitted (the file shows the default Windows exe icon).
        Ok(())
    }
