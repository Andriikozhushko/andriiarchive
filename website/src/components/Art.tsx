/**
 * ANDRII hand-drawn ink illustration primitives.
 *
 * These are ported verbatim from the desktop app's project-owned art
 * (src/components/art/index.tsx). Black-ink outlines on parchment, wax-red
 * seals, muted accents — deliberately a little irregular / handcrafted. No
...
export function ArchiveBox({
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" className={className} fill="none" aria-hidden="true">
      ...
      <path d="M44 96 q56 -10 112 0 l-4 64 q-52 9 -104 0 Z" fill={PAPER} {...strokeProps} />
...
/* ── Wax seal (intact) ────────────────────────────────────────────────────── */
export function WaxSeal({ size = 168, className = "" }: ArtProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" className={className} fill="none" aria-hidden="true">
      ...
      <path
        d="M100 38 q40 0 48 38 q6 30 -14 48 q-12 12 -34 12 q-26 0 -40 -16 q-16 -18 -10 -44 q8 -38 50 -38 Z"
        fill={WAX}
        stroke={WAX_DEEP}
        strokeWidth={3.5}
...
/* ── Small ink glyphs (inherit currentColor) — for chips & labels ─────────── */
...
function Glyph({ size = 16, className = "", children }: GlyphProps & { children: React.ReactNode }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className={className} {...g} aria-hidden="true">
      {children}
    </svg>
  );
}
